...
export function IndustryStep({ industryId, customIndustry, vibe, onChange, onNext }: Props) {
  ...
  return (
    <div>
      ...
      <p className="mt-1.5 text-[--color-muted]">Pick the field that fits best. It tailors the questions that follow.</p>
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-4 text-left transition-colors ${
                active
                  ? "border-[--color-accent] bg-[--color-paper] ring-1 ring-[--color-accent]"
                  : "border-[--color-line] bg-[--color-paper] hover:border-[--color-accent]/50 hover:bg-[--color-paper-2]"
              }`}
            >
              <div className="font-medium">{ind.name}</div>
              <div className="mt-1 text-[13px] leading-snug text-[--color-muted]">{ind.blurb}</div>
            </button>
          );
        })}
      </div>
      ...
      {isCustom && (
        <div className="mt-5">
          ...
          <input
            ...
            className="mt-1.5 w-full rounded-lg border border-[--color-line] bg-[--color-paper] px-3.5 py-2.5 text-[15px] outline-none focus:border-[--color-accent] focus:ring-1 focus:ring-[--color-accent]"
          />
        </div>
      )}
      ...
      {selected && (
        <div className="mt-6">
          <label className="text-sm font-medium">
            Describe your map in plain English <span className="text-[--color-muted]">(optional, recommended)</span>
          </label>
          <textarea
            value={vibe}
            ...
            className="mt-1.5 w-full resize-none rounded-lg border border-[--color-line] bg-[--color-paper] px-3.5 py-3 text-[15px] outline-none focus:border-[--color-accent] focus:ring-1 focus:ring-[--color-accent]"
          />
          <ul className="mt-2 space-y-1 text-[13px] text-[--color-muted]">
            {VIBE_EXAMPLES.map((e) => (
              <li key={e} className="flex gap-2">
                <span className="text-[--color-accent]">“</span>
                <button type="button" onClick={() => onChange({ vibe: e })} className="text-left hover:text-[--color-ink] hover:underline">
                  {e}
                </button>
              </li>
            ))}
